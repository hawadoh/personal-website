...
Why? Because our promise is about the *global state* of all $n$ copies. For the global state to be nearly perfect (i.e., have a high global fidelity $F_n = F_1^n$), the fidelity of each *single copy* ($F_1$) must be extremely close to $1$. This, in turn, forces the true error rate $\delta$ - which is a property of a single copy - to be comparatively much smaller than it would be in the single-copy test. This forces the thresholds for what we consider a "close" or "far" error rate to be squeezed into a much narrower window near zero.
